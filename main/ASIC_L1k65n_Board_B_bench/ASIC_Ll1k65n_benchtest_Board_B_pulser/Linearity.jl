...
include("$(@__DIR__)/$relPath/bench_test/benchtest_linearity.jl")

# plotting options!
plotFlag = true

# settings 
set = (Cinj_fF = 3000, # capacitance of the ASIC in femto Farrad |
        Rf_MOhm = 500,  # feedback resistor in Mega Ohm
        Temp_K = 300, # temperature in Kelvin
        Cf_fF = 500) #feedback capacitance 
...
begin 
    ...
    fpath = "$(@__DIR__)$(pd_data.figurefolder)Linearity/Vin-$(VinMode)_Vout-$(VoutMode)/"

    if !ispath(fpath)
        mkpath(fpath)
    end
    fname = fpath * "Linearity_" * benchtest_filename_plot(set.Cinj_fF, set.Rf_MOhm, set.Temp_K)
