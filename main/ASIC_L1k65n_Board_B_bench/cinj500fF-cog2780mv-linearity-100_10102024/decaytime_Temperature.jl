...
# load waveforms
path = ENV["ASIC_DATA"] * pd_data.datafolder 
...
begin
    ...
    timeIdx = findfirst(x -> x > ustrip.(t99_cold)[1] + 10 , ustrip.(time)) 
    ...
    plts = plot(collect(wvfs_cold[i].time), wvfs_cold[i].signal .- baseline_cold[i], linewidth = 2.5, color = :dodgerblue, 
   ...
   # plot!(time[timeIdx:end]  , tail_dsp_cold[timeIdx:end], linewidth = 2, linestyle = :dash, color = :black, label = false )
    vline!([τ_cold[i] .+ t99_cold[i]], linewidth = 1.5, linestyle = :dash, color = :dodgerblue, label = false)
