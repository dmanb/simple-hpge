...
e_grid_ft_trap              = dsp_config.e_grid_ft_trap
enc_pickoff_trap            = 200u"µs"#dsp_config.enc_pickoff_trap
...
# display results
p1 = plot(wvfs[1].time, wvfs[1].signal, label = "Raw waveform 1", 
    xlabel = "Time", ylabel = "Amplitude (a.u.)", linewidth = 2.5, color = :dodgerblue, legend = :right)

    # plot result: ENC graph 

p2 = heatmap(ustrip.(collect(e_grid_rt_trap)), ustrip.(collect(e_grid_ft_trap)), ENC_eV,  
...
p3 = plot(ustrip.(collect(e_grid_rt_trap)),  ENC_eV[ENC_idx[1],:], linestyle = :dash, color = :silver, linewidth = 2, label = false)
scatter!(ustrip.(collect(e_grid_rt_trap)),  ENC_eV[ENC_idx[1],:], 
...
plot(p1, p2,p3,  layout = (3,1), size = (900, 1500), 
    ...
    plot_title = "ASIC L1k65n Board B: bench test baseline only \n ENC pick-off time $(enc_pickoff_trap)", 
...
fname = replace(replace(fpath * "ENCcurve_baselineonly_pickofftime$(enc_pickoff_trap)_ft$(minimum(e_grid_ft_trap))-$(maximum(e_grid_ft_trap))_rt$(minimum(e_grid_rt_trap))-$(maximum(e_grid_rt_trap))"," " => ""), "." => "p") * ".png"
savefig(fname)
display(plot!())