...
wvfs, MetaData = read_folder_csv_oscilloscope(folder; heading = 17, nChannels = 1)
...
plot(p..., layout = (ceil(Int,nwvfs/3), 3), size = (nwvfs*110, nwvfs*70), 
        ...
        plot_title = "$(extraStr[2:end]) Waveforms: $(pd_data.datafolder)") 
fpath = "$(@__DIR__)" * pd_data.figurefolder * "Waveforms/"
if !ispath(fpath)
    mkpath(fpath)
end
savefig(fpath * "Waveforms$(extraStr).png")